...
// import 'package:nupura_cars/views/BookingScreen/service_booking_list_screen.dart';
// import 'package:nupura_cars/views/CarWash/carwash_screen.dart';
...
//         NavItem(icon: Icons.event_note, label: 'Car Wash'),
...
//   physics: const NeverScrollableScrollPhysics(), // looks fixed
//             child: Row(
...
import 'package:nupura_cars/views/BookingScreen/booking_screen.dart';
import 'package:nupura_cars/views/CarWash/car_wash_booking_list_screen.dart';
import 'package:nupura_cars/views/CarService/service_booking_list_screen.dart';
import 'package:nupura_cars/views/CommonBooking/common_booking_screen.dart';
import 'package:nupura_cars/views/HomeScreen/home_screen.dart';
import 'package:nupura_cars/views/ProfileScreen/profile_screen.dart';
import 'package:nupura_cars/views/guest_modal.dart';
import 'package:nupura_cars/providers/AuthProvider/auth_provider.dart';
...
class _MainNavigationScreenState extends State<MainNavigationScreen>
  ...
  // 0: Home, 1: Bookings, 2: Service Booking, 3: Car Wash, 4: Profile
  final List<Widget> _screens = const [
    HomeScreen(),
    BookingScreen(),
    // ServiceBookingListScreen(),
    CommonBookingsScreen(),
    ProfileScreen(),
  ];

  final List<NavItem> _navItems = const [
    NavItem(icon: Icons.home_filled, label: 'Home'),
    NavItem(icon: Icons.build_circle_outlined, label: 'Self Drive'),
    // NavItem(icon: Icons.event_note, label: 'Service'),
    NavItem(icon: Icons.local_car_wash, label: 'Booking'),
    NavItem(icon: Icons.account_circle, label: 'Profile'),
  ];
        ...
        return Container(
          ...
          child: Row(
            ...
            children: _navItems.asMap().entries.map((entry) {
              ...
              NavItem item = entry.value;
              bool isActive = _currentIndex == index;
              // Bookings tab (index 1) is locked for guests
              bool isLocked = authProvider.isGuest && index == 1;
  ...
  void _updateIndex(int index) {
    if (_currentIndex != index) {
      // Bookings tab is index 1 – block guests
      if (index == 1) {
        final authProvider = Provider.of<AuthProvider>(context, listen: false);
        if (authProvider.isGuest) {
          GuestLoginBottomSheet.show(context);
